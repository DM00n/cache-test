...
void Cache::experiment_d(int num) {
    ...
    auto t2 = Clock::now();
    make_report("\"direct\"", num, std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1).count());
}
...
void Cache::experiment_rev(int num) {
    ...
    auto t2 = Clock::now();
    make_report("\"reverse\"", num, std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1).count());
}
...
void Cache::experiment_rand(int num) {
    ...
    auto t2 = Clock::now();
    make_report("\"random\"", num, std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1).count());
}

void Cache::make_report(const std::string &dir, int num, int dur) {
    std::string str1, exps, expt1, expt2, expt3, duration, buf_size, number, OMEGASTRING;
    str1 = "investigation:\n"
           "\ttravel_variant: ";
    exps = "\nexperiments:\n";
    expt1 = "\t- experiment:\n\t\tnumber: ";
    ...
    if (num == 0) OMEGASTRING = str1 + dir + exps + expt1 + number + expt2 + buf_size + expt3 + duration;
    else OMEGASTRING = expt1 + number + expt2 + buf_size + expt3 + duration;
    std::ofstream out;
    out.open(R"(C:\Users\Rex\Documents\GitHub\cache_test)", std::ios::app);
    if (!out) std::cout << "error opening file";
    else out << OMEGASTRING;
    OMEGASTRING.clear();
    out.close();
}
